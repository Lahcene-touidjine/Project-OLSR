  ...
  while (N2.begin () != N2.end ())
    {
      ...
      // through this 1-hop neighbor
       ...
       std::map<int, std::vector<const NeighborTuple *> > dist_ability;
       std::set<int> da;
      ...
      for (NeighborSet::iterator it = N.begin (); it != N.end (); it++)
        {
          ...
          int r = 0;
           int d = 0; //..................somme de distance
         double d_moyen=0;//....................................distance moyene
        double var=0;//varaince
              ...
              if (nb_tuple.neighborMainAddr == nb2hop_tuple.neighborMainAddr)
                {
                  r++;
                         ...
                         if( it2 == N2.begin ())
                         {
                                d=nb2hop_tuple.Deux_s_Distance;
                          }else
                          { 
                                d=d+nb2hop_tuple.Deux_s_Distance;
                              
                          }
                                   //  sstd::cout<<"°°°°°°°°°°°°°°°°°°°°°°°°°°°°° "<<std::endl;//affichage
                                     // std::cout<<"la distance deux saut "<<  d<<std::endl;//affichage
               }  
            }
                                          if( r==0){
                                           continue;
                                   ...
                                   d_moyen=d/r;//la moyenne 
                                   var=(d_moyen)*2;
                                         std::cout<<"le degree d'accsieblité  "<<  r<<std::endl;
                                          std::cout<<" distance moyen  "<<  d_moyen<<std::endl;
                                          // std::cout<<"la varaince   "<<  var<<std::endl;
}
          rs.insert (r);
          reachability[r].push_back (&nb_tuple);

          da.insert(d_moyen);
          dist_ability[d_moyen].push_back(&nb_tuple);
       da.insert(var);
        dist_ability[var].push_back(&nb_tuple);
}
//calculer le seil le moyenne entre les distance moyenne (N1)


double moyenne=0;int nb=0;double S;
      for (std::set<int>::iterator it = da.begin (); it != da.end (); it++)
        {
          int m= *it;
          if (m== 0)
            {
          ...
          for (std::vector<const NeighborTuple *>::iterator it2 = dist_ability[m].begin ();
               it2 != dist_ability[m].end (); it2++)
            {
              ...
              if(nb_tuple ==0){
                        moyenne=m;nb++;}
              moyenne=(m+moyenne);nb++;
        //std::cout<<"la moyene "<< moyenne << " nb "<< nb <<std::endl;nb++;
              
            }
        }
        S=moyenne/nb;// .................. le seil
std::cout<< "le seil c est "<< S <<std::endl; 
//std::cout<< " nb " << nb<<std::endl;
//-----modfied ---//
////calculer le nombre de neoud n2 qui sont < = au seil
              std::map<int, std::vector<const NeighborTuple *> > ability;
              std::set<int> ra;
      for (NeighborSet::iterator it = N.begin (); it != N.end (); it++)
        {
          NeighborTuple const &nb_tuple = *it;
          ...
          for (TwoHopNeighborSet::iterator it2 = N2.begin (); it2 != N2.end (); it2++)
            {
              ...
              if (nb_tuple.neighborMainAddr == nb2hop_tuple.neighborMainAddr)
                {
                    if(nb2hop_tuple.Deux_s_Distance<=(S+200)){
                         a++;
                                                                }
                }
            }
//std::cout<<a<<""<<std::endl;
          ra.insert (a);
          ability[a].push_back (&nb_tuple);
        }

//std::cout<<"la moyene"<<moyenne<< "nb"<<nb<<"Siel "<<S<<std::endl; 
...
// cette fonction récupere l'identificateur d"un noeud. Le but de l'utilisation de cette fonction est pour avoir la position  x et y de chaque noeud a fin de choisir la variance entre deux noeuds.  
 
                 //la fonction recuperation de id node
int RoutingProtocol::get_ID ()
{
        return m_ipv4->GetObject<Node> ()->GetId ();
}
Vector RoutingProtocol::get_POS ()
{
        Ptr<Node> object = GetObject<Node> ();

        Ptr<MobilityModel> position = object->GetObject<MobilityModel> ();
        NS_ASSERT (position != 0);
// On fait appel a la fonction GetPosition pour récupérer la position d'un noeud. Le résultat est sotocké sous form un vecteur de type double .
        Vector paus = position->GetPosition ();
       
return paus;
}
// modified code
// La fonction Control_POS  affiche un noeud spécifique

void RoutingProtocol::control_POS ()
{

        if ( get_ID () == 5 ) // si vous voulez afficher seulement le nœud N° 5
        {
        std::ostringstream oss22 ;
        oss22 << " Noeud N° " << get_ID () << "";
        std::cout << oss22.str ();
      
        Vector paus = get_POS ();
        std::cout << " Position x=" << paus.x << ", y=" << paus.y << std::endl;
...
// modified code
// cette instrucion ajoute la position du neud dans un vecteur
    Vector paus = get_POS ();

//....................................... la fonction mathmatique qui calcule la distance 

// modified code
// d est la distance entre deux noeuds. Le but fnal de la distance est pour calculer la variance entre deux noeuds, afin de caluler les MPRs les plus stables; 

double d=sqrt(((hello.x1-paus.x)*(hello.x1-paus.x))+((hello.y1-paus.y)*(hello.y1-paus.y)));//fonction distance
  LinkSensing (msg, hello, receiverIface, senderIface, d);
          
std::cout<<"recuperer Hello de "<<senderIface<<" position  x....> "<<hello.x1<<" position y....> "<<hello.y1<<std::endl;
std::cout<<"vers "<<receiverIface<<"position  x....>  "<<paus.x<<" position  y....> "<<paus.y<<std::endl;
        // printf("............ .................................................................\n"); 
        //std::cout << " Position of node  N° " << get_ID () << " recevier_hello      x =     " << paus.x << " ,       y= " << paus.y << std::endl;  //----affichage de  laposition de recepteur de hello
        //std::cout << "From node  who has the  Position          X=    " << hello.x1 << ",            Y=    " << hello.y1 << std::endl; //---- position node l'send hello

    //std::cout << "the distance between nodes =" << d << std::endl;//affichage 
...
// modified code
// La fonction sendHello permet d'un noeud mobile d'envoyer un message vers un noeud fixe.
...
// modified code
// On ajout un veteur qui contient le message hello pour pouvoir récupérer sa position (la position de l'emetteur). 


// vecteur paus
 Vector paus = get_POS ();

 // l'affectation des x et y dans x1 et y1

hello.x1=(paus.x); 
hello.y1=(paus.y);

std::cout<<"envoi hello par  ..........position (x):"<<paus.x<<" ..........position (y):"<<paus.y<<"  address de "<<m_mainAddress<<std::endl; 
...

    {
      ...
      linkMessage.linkCode = (link_type & 0x03) | ((nb_type << 2) & 0x0f);
       linkMessage.neighborInterfaceAddresses.push_back
        (link_tuple->neighborIfaceAddr);
//............................. hadi hiyaa li thaseb la distances
     linkMessage.stable_distance=(link_tuple->D);//.... dans la liste des voisin assosier la distance  a chaque neighborIfaceAddr "link_tuple->D"
  ...
  for (MprSelectorSet::const_iterator mprsel_tuple = m_state.GetMprSelectors ().begin ();
       mprsel_tuple != m_state.GetMprSelectors ().end (); mprsel_tuple++)
    {
      ...
      tc.neighborAddresses.push_back (mprsel_tuple->mainAddr);
        tc.stable_distance =mprsel_tuple->mpr_distance;
    }
  QueueMessage (msg, JITTER);
}
...
RoutingProtocol::LinkSensing (const olsr::MessageHeader &msg,
                              ...
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface,double d)
{
  Time now = Simulator::Now ();
  bool updated = false;
  bool created = false;
...
//................modified code remplir le champ D de link typle avec la distance recevie dans le "hello"
    ...
    {
//............modified code mis à jour le champ D de link typle avec la distance recevie dans le "hello"
      link_tuple->D=d;
 ...
 // la fonction PopulateNeighborSet  
 // Tuple c'est un coupe de deux noeuds qui peuvent se c ommuniquer entre eux.
// 
void
...
//................................... modified code  ..............
      ...
      Vector paus = get_POS ();// vecteur 
double d=sqrt(((hello.x1-paus.x)*(hello.x1-paus.x))+((hello.y1-paus.y)*(hello.y1-paus.y)));// fonction distance

// Tab_D c est  un pointeur sur nb_tuple qui recupere le couple de noeud a fin de sauvgrader la distance entres eux dans un tableau .
nb_tuple->Tab_D= d;   // association entre la fonction et le tebleau 

//std::cout<<"distance un saut   :"<<nb_tuple->Tab_D<< std::endl;// affichage 
...
double dist=0;
if(GetMainAddress (link_tuple->neighborIfaceAddr) == msg.GetOriginatorAddress ()){
                        dist=linkMessage->stable_distance;}
                 new_nb2hop_tuple.Deux_s_Distance=dist;
      ...
      if (nt == OLSR_MPR_NEIGH)
        {
         ...
         mprsel_tuple.mpr_distance=linkMessage->stable_distance;
