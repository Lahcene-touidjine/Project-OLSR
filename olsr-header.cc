...
MessageHeader::GetSerializedSize (void) const
{
  ...
  switch (m_messageType)
    ...
    case HELLO_MESSAGE:
      ...
      //NS_LOG_UNCOND("Hello Message Size: " << size << " + " << m_message.hello.GetSerializedSize ());
...
{
 // la modification de la taille du packet pour pouvoir ajouter es autres chapms: les Xs et les Ys ; les position du neuds est inclu dans le packet.
...
// modified code
          ...
          // ajouter less nouveaux parametres (x, y et d  (disance)). Pour ecrire les nouveux chapms dans le packet.      
  i.WriteU16 (0); // Reserved
  i.WriteU8 (this->hTime);
  i.WriteU8 (this->willingness);
                                      //.......................... on ajoute ou declarer les variable ...............
i.WriteU8 (this->x1);
i.WriteU8 (this->y1);
i.WriteU8 (this->Dist);
i.WriteU8 (this->Var);
  for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
       iter != this->linkMessages.end (); iter++)
    {
      ...
      i.WriteHtonU16 (8 + lm.neighborInterfaceAddresses.size () * IPV4_ADDRESS_SIZE);
...
uint64_t
MessageHeader::Hello::Deserialize (Buffer::Iterator start, uint64_t messageSize)
{
  Buffer::Iterator i = start;
...
// Pour lire les valeurs (x,y et d) saisie dans le champs.      
  
        //.................. ajouter les variables dans deserialize 
this->x1= i.ReadU8 ();
this->y1= i.ReadU8 ();
this->Dist= i.ReadU8 ();
this->Var= i.ReadU8 ();
  ...
  while (helloSizeLeft)
    {
      ...
      NS_ASSERT (helloSizeLeft >= 8);
       //................................ on associer linkMessage avec Readu8
      lm.linkCode = i.ReadU8 ();
                
        
      i.ReadU8 (); // Reserved
      uint16_t lmSize = i.ReadNtohU16 ();
      NS_ASSERT ((lmSize - 8) % IPV4_ADDRESS_SIZE == 0);
      for (int n = (lmSize - 8) / IPV4_ADDRESS_SIZE; n; --n)
        {
          lm.neighborInterfaceAddresses.push_back (Ipv4Address (i.ReadNtohU32 ()));
         lm.stable_distance=i.ReadU16 ();
             //  i.ReadU16(); 
        }
...
{
  return 8 + this->neighborAddresses.size () * IPV4_ADDRESS_SIZE;
}
...
MessageHeader::Tc::Serialize (Buffer::Iterator start) const
{
 ...
 i.WriteU16(this->stable_distance);
 //
...
uint64_t
MessageHeader::Tc::Deserialize (Buffer::Iterator start, uint64_t messageSize)
{
  Buffer::Iterator i = start;
...
this->stable_distance = i.ReadU16 ();
  //
  i.ReadNtohU16 (); // Reserved

  NS_ASSERT ((messageSize - 8) % IPV4_ADDRESS_SIZE == 0);
  int numAddresses = (messageSize - 8) / IPV4_ADDRESS_SIZE;
  this->neighborAddresses.clear ();
  for (int n = 0; n < numAddresses; ++n)
    {
